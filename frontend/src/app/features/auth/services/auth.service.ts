...
export class AuthService {
  ...
  logout(): void {
    ...
    this.http.post(`${environment.apiUrl}/api/logout`, {}, { withCredentials: true }).subscribe({
      next: () => {
        ...
        setTimeout(() => this.isInitializingAuth$.next(false), 4200); // Désactive après un petit Délai minimum (friction positive pour l'écran de chargement)
      },
      error: (error) => {
  ...
  initializeAuth(): Observable<boolean> {
    ...
    if (!accessToken) {
      if (rememberMe) {
        ...
        return this.refreshAccessToken().pipe(
          ...
          tap(() => setTimeout(() => this.isInitializingAuth$.next(false), 4200)) // Délai minimum (friction positive pour l'écran de chargement) après un refresh réussi ou échec
        );
      } else {
    ...
    this.loggedIn$.next(true);
    this.isInitializingAuth$.next(false); 
    return of(true); 
  }
}
